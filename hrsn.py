...
class HRSN():
    """
        Hierarchical Recurrent Siamese Network (HRSN) topology for (binary) authorship verification.
    ...
    def __init__(self, hyper_parameters, E_w_init):
        ...
        with tf.variable_scope('feature_extraction_doc2vec'):
            ...
            # document embeddings
            e_d = self.feature_extraction(e_w, N_w, N_s)
    ...
    ################################################
    def feature_extraction(self, e_w, N_w, N_s):

        with tf.variable_scope('words_to_sentence_encoding'):
            e_s = self.bilstm_layer_ws(e_w, N_w)
        with tf.variable_scope('sentences_to_document_encoding'):
            e_d = self.bilstm_layer_sd(e_s, N_s)
    ...
    def bilstm_layer_ws(self, e_w_f, N_w):
        ...
        # reshape input word embeddings, shape = [2 * B * T_s, T_w, D_w]
        e_w_f = tf.reshape(e_w_f, shape=[2 * self.B * T_s, T_w, D_w])
        ...
        # concatenate hidden states, shape=[2 * B * T_s, 2 * D_s]
        h = tf.concat([h_f, h_b], axis=1)
        # reshape input word embeddings, shape = [2 * B, T_s, 2 * D_s]
        h = tf.reshape(h, shape=[2 * self.B, T_s, 2 * D_s])
    ...
    def bilstm_layer_sd(self, e_s_f, N_s):
        ...
        # concatenate hidden states, shape=[2 * B, 2 * D_d]
        h = tf.concat([h_f, h_b], axis=1)
    ...
    def initialize_dropout(self):
        ...
        with tf.variable_scope('dropout_lstm_ws_forward'):
            ...
            dropout['lstm_ws_forward']['x'] = self.make_dropout_mask(shape=[self.B * T_s, D_w],
        ...
        with tf.variable_scope('dropout_lstm_ws_backward'):
            ...
            dropout['lstm_ws_backward']['x'] = self.make_dropout_mask(shape=[self.B * T_s, D_w],
                                                                      keep_prob=self.hyper_parameters['keep_prob_lstm'],
                                                                      )
    ...
    def initialize_parameters(self):
        ...
        with tf.variable_scope('theta_lstm_ws_forward'):
            theta['lstm_ws_forward'] = self.initialize_lstm(D_w, D_s)
        with tf.variable_scope('theta_lstm_ws_backward'):
            theta['lstm_ws_backward'] = self.initialize_lstm(D_w, D_s)
    ...
    #############################
    def update_model(self, x_w_L, x_w_R, labels, N_w_L, N_w_R, N_s_L, N_s_R, lr):
    ...
    def evaluate_model(self, docs_L, docs_R, labels, batch_size):
        ...
        for i in range(num_batches):
            ...
            if B > 0:
                ...
                x_w_L, N_w_L, N_s_L = self.doc2mat(docs_L_i)
                x_w_R, N_w_R, N_s_R = self.doc2mat(docs_R_i)
    ...
    ###############################################
    def compute_eval_measures(self, x_w_L, x_w_R, labels, N_w_L, N_w_R, N_s_L, N_s_R):
